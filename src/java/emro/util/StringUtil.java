...
public class StringUtil {
	...
	public static String formatNum(String numStr) {
		try {
			if(numStr != null || !"".equals(numStr)) {
				numStr = StringUtil.replace(numStr, "," , "" );
		    	if( numStr.indexOf(".") != -1 )
		    		return NumberFormat.getInstance().format((new java.lang.Double(numStr)).doubleValue());
		    	else {
		    		return NumberFormat.getInstance().format((new java.lang.Long(numStr)).longValue());
		    	}
			}else {
				numStr = "";
			}
			...
			for(int i=0;i<numStr.length;i++){
				if(numStr[i] != null || !"".equals(numStr[i])) {
			    	...
			    	if( numStr[i].indexOf(".") != -1 )
			    		numStr[i] = NumberFormat.getInstance().format((new java.lang.Double(numStr[i])).doubleValue());
			    	else {
			    		numStr[i] = NumberFormat.getInstance().format((new java.lang.Long(numStr[i])).longValue());
			    	}
				}else {
					numStr[i] = "";
				}
	...
	public static String cleanNum(String numStr) {
		try {
			...
			throw new NumberFormatException("Parsing Failure : " + numStr);
		}
	}
	...
	*/
	...
	public static String removeAndreplace(String input, List removings, String inserting) {
		StringBuffer sb = new StringBuffer(input);
		for( Iterator i = removings.iterator(); i.hasNext(); ) {
			String removing = (String)i.next();
			while( sb.toString().indexOf(removing) != -1 ) {
				sb.replace( sb.toString().indexOf(removing), sb.toString().indexOf(removing)+removing.length(), inserting);
			}
		}
		return sb.toString();
	}
	 ...
	 */
	private static boolean isNormalString(String input) {
 		if (input == null) {
 			return true;
 		}
 		int abnormalCount = 0;
		...
		if (questionCount > normalCount / 2) {
			return false;
		}
		return true;
	}
   ...
   public static double convertToDbl(String source, double defaultValue) {
       ...
       try {
           ...
           for (int ii = 0; ii < safeLength(source); ii++) {
               if (source.startsWith("0") && source.startsWith("0.") == false) {
                   source = source.substring(1);
               } else {
                   break;
               }
           }
           result = (new Double(convertToStrTrim(source))).doubleValue();
